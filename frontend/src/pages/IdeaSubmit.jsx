...
import { saveSession } from '../hooks/useSessionData';
...
export default function IdeaSubmit() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Show a clear, accurate error message
      const status = err.response?.status;
      if (status === 429) {
        setError('AI is busy right now (rate limited). Please wait 30 seconds and try again.');
      } else if (!err.response) {
        setError('Cannot reach the server. Make sure the backend is running on port 5000.');
      } else {
        setError(err.response?.data?.error || 'Failed to analyze idea. Please try again.');
      }
      setPhase('input');
    } finally {
      setLoading(false);
  ...
  const handleContinue = () => {
    saveSession({ problem, idea, domain, hiddenCons, iterationCount: 1 });
    navigate('/engines', {
      state: { problem, idea, domain, hiddenCons, iterationCount: 1 },
    });
  };
