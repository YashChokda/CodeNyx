...
export default function MainEngines() {
  ...
  const problem = location.state?.problem || session.problem || '';
  const idea = location.state?.idea || session.idea || '';
  const domain = location.state?.domain || session.domain || '';
  const iterationCount = location.state?.iterationCount || session.iterationCount || 1;
  ...
  const fetchAllEngines = async () => {
    ...
    const ripplePromise = axios.post('/api/gemini/ripple', { problem, idea, domain })
      .then(res => {
        ...
        saveSession({ rippleData: res.data });
        return res.data;
      })
      .catch(err => {
    ...
    const stakeholderPromise = axios.post('/api/stakeholders', { problem, idea, domain })
      .then(res => {
        ...
        saveSession({ stakeholderData: res.data.stakeholders });
      })
      .catch(err => {
    ...
    const mentorPromise = ripplePromise.then(rippleResult => {
      ...
      return axios.post('/api/gemini/mentor', { problem, idea, domain, rippleScores })
        .then(res => {
          ...
          saveSession({ mentorData: res.data });
        })
        .catch(err => {
  ...
  const handleContinueToRefine = () => {
    navigate('/refine', {
      state: { problem, idea, domain, rippleData, mentorData, stakeholderData, iterationCount },
    });
  };
    ...
    <div className="min-h-screen relative overflow-hidden flex flex-col" style={{ background: '#0d0d1a' }}>
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => navigate('/dashboard', { state: { problem, idea, domain, rippleData, mentorData, stakeholderData, iterationCount } })}
      ...
      {/* Main Content Area */}
      <div className="relative z-10 flex-1 w-full overflow-y-auto px-5 sm:px-8 pb-24">
